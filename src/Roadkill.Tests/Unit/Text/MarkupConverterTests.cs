...
namespace Roadkill.Tests.Unit.Text
{
	...
	public class MarkupConverterTests
	{
		...
		public void Setup()
		{
			...
			_configuration.UseHtmlWhiteList = true;
		...
		{
			// Arrange, act
			_configuration.MarkupType = "Creole";
			...
			_configuration.MarkupType = "Markdown";
			_markupConverter = new MarkupConverter(_configurationStore, _pageRepository, _pluginFactory);
			Assert.NotNull(_markupConverter.Parser);
		}
		...
		public void html_should_not_be_sanitized_if_usehtmlwhitelist_setting_is_false()
		{
			...
			MarkupConverter converter = new MarkupConverter(_configurationStore, _pageRepository, _pluginFactory);
